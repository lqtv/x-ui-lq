...
if [ ! -f xuilq_update ]; then
green "首次安装x-ui-lq脚本必要的依赖……"
...
green "下载并安装x-ui相关组件……"
cd /usr/local/
#curl -sSL -o /usr/local/x-ui-linux-${cpu}.tar.gz --insecure https://gitlab.com/rwkglq/x-ui-lq/raw/main/x-ui-linux-${cpu}.tar.gz
curl -sSL -o /usr/local/x-ui-linux-${cpu}.tar.gz --insecure https://raw.githubusercontent.com/leqingtv/x-ui-lq/main/x-ui-linux-${cpu}.tar.gz
...
cd
#curl -sSL -o /usr/bin/x-ui --insecure https://gitlab.com/rwkglq/x-ui-lq/raw/main/1install.sh >/dev/null 2>&1
curl -sSL -o /usr/bin/x-ui --insecure https://raw.githubusercontent.com/leqingtv/x-ui-lq/main/install.sh >/dev/null 2>&1
...
echo "----------------------------------------------------------------------"
restart
#curl -sL https://gitlab.com/rwkglq/x-ui-lq/-/raw/main/version/version | awk -F "更新内容" '{print $1}' | head -n 1 > /usr/local/x-ui/v
curl -sL https://raw.githubusercontent.com/leqingtv/x-ui-lq/main/version | awk -F "更新内容" '{print $1}' | head -n 1 > /usr/local/x-ui/v
xuilogin(){
v4v6
if [[ -z $v4 ]]; then
...
cronxui
echo "----------------------------------------------------------------------"
blue "x-ui-lq $(cat /usr/local/x-ui/v 2>/dev/null) 安装成功，自动进入 x-ui 显示管理菜单" && sleep 4
echo
show_menu
}
...
yellow "一、点击x-ui面版中的备份与恢复，下载备份文件x-ui-lq.db"
yellow "二、在 /etc/x-ui-lq 路径导出备份文件x-ui-lq.db"
...
green "x-ui已卸载完成"
echo
blue "欢迎继续使用x-ui-lq脚本：bash <(curl -Ls https://raw.githubusercontent.com/leqingtv/x-ui-lq/main/install.sh)"
echo
else
red "输入有误" && uninstall
fi
...
acme() {
bash <(curl -Ls https://gitlab.com/rwkglq/acme-script/raw/main/acme.sh)
back
}
...
cfwarp() {
bash <(curl -Ls https://gitlab.com/rwkglq/CFwarp/raw/main/CFwarp.sh)
back
}
...
#curl -L -o /usr/local/x-ui/cloudflared -# --retry 2 https://gitlab.com/rwkglq/sing-box-lq/-/raw/main/$cpu
...
xip1=$(cat /usr/local/x-ui/xip 2>/dev/null | sed -n 1p)
ymip=$(cat /root/leqingca/ca.log 2>/dev/null)
...
show_menu(){
clear
white white "乐青Github项目  ：github.com/leqingtv"
white "乐青Blogger博客 ：leqing.blogspot.com"
white "乐青YouTube频道 ：www.youtube.com/@leqing"
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
white "x-ui-lq脚本快捷方式：x-ui"
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
green " 1. 一键安装 x-ui"
green " 2. 删除卸载 x-ui"
echo "----------------------------------------------------------------------------------"
...
#latestV=$(curl -s https://gitlab.com/rwkglq/x-ui-lq/-/raw/main/version/version | awk -F "更新内容" '{print $1}' | head -n 1)
latestV=$(curl -sL https://raw.githubusercontent.com/leqingtv/x-ui-lq/main/version | awk -F "更新内容" '{print $1}' | head -n 1)
if [[ -f /usr/local/x-ui/v ]]; then
if [ "$insV" = "$latestV" ]; then
echo -e "当前 x-ui-lq 脚本最新版：${bblue}${insV}${plain} (已安装)"
else
echo -e "当前 x-ui-lq 脚本版本号：${bblue}${insV}${plain}"
echo -e "检测到最新 x-ui-lq 脚本版本号：${yellow}${latestV}${plain} (可选择6进行更新)"
echo -e "${yellow}$(curl -sL https://raw.githubusercontent.com/leqingtv/x-ui-lq/main/version)${plain}"
#echo -e "${yellow}$(curl -sL https://gitlab.com/rwkglq/x-ui-lq/-/raw/main/version/version)${plain}"
fi
else
echo -e "当前 x-ui-lq 脚本版本号：${bblue}${latestV}${plain}"
echo -e "请先选择 1 ，安装 x-ui-lq 脚本"
fi
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
echo -e "VPS状态如下："
...
if [[ -f /root/leqingca/ca.log ]]; then
echo -e "$blue路径域名登录地址(安全)：https://$(cat /root/leqingca/ca.log 2>/dev/null):${xport}${xpath}$plain"
fi
fi
else
echo -e "x-ui登录信息如下："
